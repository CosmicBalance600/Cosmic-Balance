...
class EarthGlobe {
    ...
    createSatellites() {
        const satellitePositions = [
            { x: 1.5, y: 0, z: 0, name: 'DSCOVR' },
            { x: -1.5, y: 0, z: 0, name: 'ACE' },
            { x: 0, y: 1.8, z: 0, name: 'GOES-16' },
            { x: 0, y: -1.8, z: 0, name: 'SDO' }
        ];
        ...
        satellitePositions.forEach(pos => {
            ...
            const bodyGeometry = new THREE.BoxGeometry(0.08, 0.05, 0.05, 8, 8, 8);
            ...
            const boxGeometry = new THREE.BoxGeometry(0.02, 0.015, 0.015, 2, 2, 2);
            ...
            const detailGeometry = new THREE.BoxGeometry(0.081, 0.015, 0.051);
            ...
            for (let x = -0.03; x <= 0.03; x += 0.015) {
                for (let z = -0.02; z <= 0.02; z += 0.02) {
                    const rivet = new THREE.Mesh(rivetGeometry, rivetMaterial);
                    rivet.position.set(x, 0.015, z);
            ...
            const panelGeometry = new THREE.BoxGeometry(0.12, 0.06, 0.005, 12, 12, 1);
            ...
            const armGeometry = new THREE.CylinderGeometry(0.003, 0.003, 0.02, 16);
            ...
            leftArm.position.set(-0.05, 0, 0);
            leftArm.rotation.z = Math.PI / 2;
            satelliteGroup.add(leftArm);
            
            // Left panel frame with detail
            const frameGeometry = new THREE.BoxGeometry(0.122, 0.062, 0.003);
            ...
            leftFrame.position.set(-0.1, 0, -0.004);
            satelliteGroup.add(leftFrame);
            
            // Add solar cell grid lines on left panel
            const lineGeometry = new THREE.BoxGeometry(0.12, 0.001, 0.006);
            ...
            rightArm.position.set(0.05, 0, 0);
            rightArm.rotation.z = Math.PI / 2;
            satelliteGroup.add(rightArm);
            
            // Right panel frame
            const rightFrame = new THREE.Mesh(frameGeometry, frameMaterial);
            rightFrame.position.set(0.1, 0, -0.004);
            ...
            const mastGeometry = new THREE.CylinderGeometry(0.006, 0.008, 0.04, 32);
            ...
            for (let i = 0; i < 3; i++) {
                const ringGeometry = new THREE.TorusGeometry(0.008, 0.001, 16, 32);
                ...
                ring.position.set(0, 0.03 + (i * 0.015), 0);
                ring.rotation.x = Math.PI / 2;
                satelliteGroup.add(ring);
            }
            
            // Main dish antenna - ultra detailed
            const dishGeometry = new THREE.CylinderGeometry(0.025, 0.02, 0.015, 64);
            ...
            dish.position.set(0, 0.07, 0);
            satelliteGroup.add(dish);
            
            // Dish inner surface with detail
            const dishInnerGeometry = new THREE.CircleGeometry(0.022, 64);
            ...
            dishInner.position.set(0, 0.0775, 0);
            dishInner.rotation.x = -Math.PI / 2;
            satelliteGroup.add(dishInner);
            
            // Dish support struts
            for (let i = 0; i < 4; i++) {
                const strutGeometry = new THREE.CylinderGeometry(0.001, 0.001, 0.02, 8);
            ...
            const hornGeometry = new THREE.ConeGeometry(0.004, 0.01, 16);
            ...
            for (let i = 0; i < 4; i++) {
                ...
                // Antenna tips
                const tipGeometry = new THREE.SphereGeometry(0.003, 16, 16);
            ...
            // Sensors and cameras
            const sensorGeometry = new THREE.CylinderGeometry(0.004, 0.004, 0.008, 16);
            ...
            frontSensor.position.set(0, 0, 0.03);
            frontSensor.rotation.x = Math.PI / 2;
            satelliteGroup.add(frontSensor);
            
            // Sensor lens
            const lensGeometry = new THREE.CircleGeometry(0.003, 16);
            ...
            const ledPositions = [
                { x: 0.035, y: 0, z: 0.026, color: 0x00ff00 },
                { x: -0.035, y: 0, z: 0.026, color: 0x00ff00 },
                { x: 0, y: 0.022, z: 0.026, color: 0xff0000 },
                { x: 0, y: -0.022, z: 0.026, color: 0xffff00 }
            ];
            
            ledPositions.forEach(led => {
                const lightGeometry = new THREE.SphereGeometry(0.003, 16, 16);
            ...
            const radiatorGeometry = new THREE.BoxGeometry(0.015, 0.04, 0.002);
